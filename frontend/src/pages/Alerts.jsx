...
const [alerts, setAlerts] = useState([]);
  ...
  async function loadAlerts() {
    try { const { data } = await api.get('/alerts'); setAlerts(data); setError(''); }
  ...
  async function resolveAlert(id) {
    try { await api.patch(`/alerts/${id}/resolve`); setError(''); await loadAlerts(); }
    ...
    <div className="page-shell">
      ...
      <section className="data-grid">
        <form className="panel data-form" onSubmit={submitReport}>
          ...
          <label className="wide-field">What happened?<textarea rows="5" maxLength={4000} value={report.description} onChange={(event) => setReport({ ...report, description: event.target.value })} placeholder="Include the request, merchant, link, or message. Never enter OTPs, PINs, passwords, or account numbers." required /></label>
